...
public class xuefengController {
    ...
    public String xuefeng(@ModelAttribute("studentId") Integer s, Model model){
        ...
        //获取同年级班级号及班级名
        List<T2StudentInfo> studentInfoList = studentInfoService.getAllStudentInfo();
        for(T2StudentInfo studentInfo1 : studentInfoList) {
            int classId = studentInfo1.getClaId();
    ...
    private void showXuefeng1(Model model, int classIndex, List<Integer> classIds){
        //数据库获取同年级的成绩、考勤数据
        List<T5Chengji> chengjiList = chengjiService.getAllChengjiFromClaIds(classIds);//保证了成绩信息一定在classId中，后面无需判断
        List<T3Kaoqin> kaoqinList = kaoqinService.getAllChengjiFromClaIds(classIds);//同上
        ...
        for(T5Chengji chengji : chengjiList){
            int index = classIds.indexOf(chengji.getClaId());//班级号位置
            int subId = Format.formatSubId(chengji.getMesSubId());//将课程号映射到0-9
            if(index < 0 || subId < 0 || subId > 9){
                continue;
            }
            Integer examNum = chengji.getExamNumber();
            int score = chengji.getMesScore();
    ...
    private void getKaoqin(Model model, List<T3Kaoqin> kaoqinList, List<Integer> classIds) {
        //考勤情况——start
        ...
        for(T3Kaoqin kaoqin : kaoqinList){
            ...
            //得到本班所有考勤情况
            String[] splitTemp = kaoqin.getDatadatetime().split(" ");
